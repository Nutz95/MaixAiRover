"""Derive HUD values from ESP / Yahboom telemetry (heading, batteries, attitude)."""
...
from lib.ui.pitch_roll_deg import PitchRollDeg

# 3S LiPo pack on Yahboom / Waveshare 12 V rail.
LIPO_EMPTY_MV = 10500
...
class HudInstruments:
  ...
  bus_mv: int | None
  maix_battery_pct: int | None = None
...
def from_telem(telem, maix_pct: int | None = None) -> HudInstruments:
  """Build HUD instruments from a TelemSnapshot (or None)."""
  if telem is None:
    return HudInstruments(None, None, None, None, None, None, maix_pct)
  heading = None
  cardinal = None
  if telem.has_mag:
  ...
  if telem.has_ina:
    ...
    pct = battery_pct(bus)
  return HudInstruments(heading, cardinal, pitch, roll, pct, bus, maix_pct)


def from_yahboom(
  attitude,
  rover_battery,
  maix_pct: int | None = None,
) -> HudInstruments:
  """Build HUD instruments from Yahboom IMU + pack voltage."""
  if attitude is None and rover_battery is None:
    return HudInstruments(None, None, None, None, None, None, maix_pct)
  heading = None
  cardinal = None
  pitch = None
  ...
  if rover_battery is not None:
    bus = int(rover_battery.millivolts)
    pct = battery_pct(bus)
  return HudInstruments(heading, cardinal, pitch, roll, pct, bus, maix_pct)
