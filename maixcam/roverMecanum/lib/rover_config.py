...
class RoverConfig:
  ...
  axis_sensitivity_percent: int = 70
  axis_expo: float = 2.2
  axis_curve: str = "log"
  speed_step: int = 5
  ...
  def from_mapping(cls, raw: dict) -> "RoverConfig":
    ...
    if not isinstance(raw, dict):
      raw = {}
    curve = str(raw.get("axis_curve", "log")).strip().lower()
    ...
    return cls(
      ...
      axis_sensitivity_percent=max(
        1, min(100, int(raw.get("axis_sensitivity_percent", 70))),
      ),
      axis_expo=max(0.3, min(3.0, float(raw.get("axis_expo", 2.2)))),
      axis_curve=curve,
